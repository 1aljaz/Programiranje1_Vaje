...
def narascajoc(tabela):
    """Vrne True, ce je tabela narascajoca, drugace False"""
...
def st_nepadajocih_cet(tabela):
    """Vrne st. nepadajocih cet v zaporedju"""
    if len(tabela) == 0:
        return 0
    st_nepadajocih = 1
    for i in range(1, len(tabela)):
...
def st_narascajocih_cet(tabela):
    """Vrne stevilo narascajocih cet v zaporedju"""
    if len(tabela) == 0:
        return 0
    st_nepadajocih = 1
    for i in range(1, len(tabela)):
...
def dolzina_najdaljse_narascajoce_cete(tabela):
    """Vrne dolzino najdaljse narascajoce cete v zaporedju"""
    if len(tabela) == 0:
        return 0
    m = -1
    temp = 1
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDU1MywidXNlciI6OTc4NH0:1tV8mJ:yg8m4D4pocCezTJmyLpKf52s3xtPf-A-e38umNIDNqs"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDU1NCwidXNlciI6OTc4NH0:1tV8mJ:1_7LLHOttJTHCDkyzSp5M5CiUZnLnKXSaOmx9156UZk"
        try:
            s = Check.current_part['solution']
            sw = re.sub('[ \n]', '', s)
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDU1NSwidXNlciI6OTc4NH0:1tV8mJ:pmXUiK2tc8FiHA0gvsI2IA80YDgwdQheb_ewXAig1nA"
        try:
            s = Check.current_part['solution']
            sw = re.sub('[ \n]', '', s)
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDU1NiwidXNlciI6OTc4NH0:1tV8mJ:yuUdE1rNd5VBf5DSsNKU8TQwABHNKJlJeB43HAVpgdE"
        try:
            s = Check.current_part['solution']
            sw = re.sub('[ \n]', '', s)
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDU1NywidXNlciI6OTc4NH0:1tV8mJ:s8m51j1fSMankE9OeqkFRwsFgyDFuEp4SGa4tuBuLUU"
        try:
            s = Check.current_part['solution']
            sw = re.sub('[ \n]', '', s)
