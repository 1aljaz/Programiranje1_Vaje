...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDM3MCwidXNlciI6OTc4NH0:1t0b0n:LmH07NmiDvC-qObzzV2vW6Z9mJGYlBpAk-d4j2tOz50"
        try:
            def izpis(source):
                import io, sys
                old_stdout = sys.stdout
            ...
            dovoljeni = "*.#$@oO+="
            znak = "*"
            preveriSmrecico(izpis(Check.current_part["solution"]), 8 * [0], range(1, 9), "*")
        except TimeoutError:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDM3MSwidXNlciI6OTc4NH0:1t0b0n:tpztFHPFh0JdGQZYfxu0qVCHACuyfK73ukpWtr3RoCo"
        try:
            def izpis(source):
                import io, sys
                old_stdout = sys.stdout
            ...
            dovoljeni = "*.#$@oO+="
            source=Check.current_part["solution"]
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDM3MiwidXNlciI6OTc4NH0:1t0b0n:sXo4tZF2Lr4mPAtlOidgZ6yfLplR23FB8fRAuIIzzyI"
        try:
            def izpis(source):
                import io, sys
                old_stdout = sys.stdout
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDM3MywidXNlciI6OTc4NH0:1t0b0n:JicD4lHL0r2dloEFEsbTlDSYd90Mr4_YF_PdmIKxZi8"
        try:
            def izpis(source):
                import io, sys
                old_stdout = sys.stdout
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDM3NCwidXNlciI6OTc4NH0:1t0b0n:dHLiJwap67-uPF5kfIXVjTbSx0Xf1ZnjxkLWZyjuEig"
        try:
            def izpis(source):
                import io, sys
                old_stdout = sys.stdout
