...
def seznam_vseh_podmap(zacetna):
    vsi = [zacetna]
    try:
        vsebina = os.listdir(zacetna)
    except PermissionError:
        return ['Faliure']
    
    for ime in vsebina:
        pot = os.path.join(zacetna, ime)
        if os.path.isdir(pot):
            vsi += seznam_vseh_podmap(pot) 
    return vsi
print(len(seznam_vseh_podmap(path))) # Ker je datotek preprosto prevec, da bi jih izpisal kot pricakovan izpis, bom preveril le koliko je takih. Pricakovan izpis: 666
# =====================================================================@040789=
# 2. podnaloga
