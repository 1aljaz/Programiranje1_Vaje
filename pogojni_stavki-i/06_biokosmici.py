...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDQzMywidXNlciI6OTc4NH0:1t9SWO:b4tFEWlwMc7f0UdmDl6qVa13mmnrmklqZJ9Xnkba5Wg"
        try:
            ...
            elif not nakup.__doc__ or nakup.__doc__ == "" or not nakup.__doc__.strip():
                Check.error("Opis funkcije ne sme biti prazen.")
                
            Check.equal("""nakup(4, 10, 3)""", 2) and \
            Check.equal("""nakup(4, 11, 3)""", 3) and \
            Check.equal("""nakup(10, 10, 4)""", 0) and \
            Check.equal("""nakup(50, 100, 3)""", 17) and \
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDQzNCwidXNlciI6OTc4NH0:1t9SWO:xsdnXUbQof0cEUnTza7_W6o3DTwzcKFr9J8AdEFYWUk"
        try:
            s = Check.current_part['solution']
            sw = re.sub('[ \n]', '', s)
            ...
            if "):\"" not in sw and "):'" not in sw:
                Check.error("Ne pozabi na opis funkcije.")
            elif not napolni.__doc__ or napolni.__doc__ == "" or not napolni.__doc__.strip():
                Check.error("Opis funkcije ne sme biti prazen.")
                
            Check.equal("""napolni(100, 40, 65, 2)""", (60, 0, 0)) and \
            Check.equal("""napolni(300, 0, 65, 3)""", (0, 60, 10)) and \
            Check.equal("""napolni(300, 150, 100, 5)""", (100, 0, 25)) and \
            Check.equal("""napolni(800, 200, 250, 10)""", (0, 0, 0)) and \
            Check.equal("""napolni(800, 200, 250, 20)""", (800, 200, 250)) and \
            Check.equal("""napolni(800, 200, 250, 11)""", (80, 20, 25)) and \
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDQzNSwidXNlciI6OTc4NH0:1t9SWO:rHol4RDcUkib5wGdNqvMnFl4a6e8d5O5KyFYc3_KFmU"
        try:
            s = Check.current_part['solution']
            sw = re.sub('[ \n]', '', s)
            ...
            Check.equal("""napolni2(100, 40, 65)""", 3) and \
            Check.equal("""napolni2(0, 0, 0)""", 0) and \
            Check.equal("""napolni2(1, 0, 0)""", 1) and \
            Check.equal("""napolni2(0, 1, 0)""", 1) and \
            Check.equal("""napolni2(0, 0, 1)""", 1) and \
            Check.equal("""napolni2(300, 0, 65)""", 4) and \
            Check.equal("""napolni2(240, 50, 50)""", 3) and \
            Check.equal("""napolni2(79, 19, 24)""", 1) and \
            Check.equal("""napolni2(81, 21, 26)""", 2) and \
            Check.equal("""napolni2(159, 21, 26)""", 2) and \
