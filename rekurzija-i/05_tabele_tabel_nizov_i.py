...
def skupna_dolzina(ttn):
    if isinstance(ttn, str):
        ...
        return sum(skupna_dolzina(s) for s in ttn)
# =====================================================================@040726=
# 2. podnaloga
...
def max_dolzina(ttn):
    if isinstance(ttn, str):
        ...
        return max(max_dolzina(s) for s in ttn)
# =====================================================================@040727=
# 3. podnaloga
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDcyNSwidXNlciI6OTc4NH0:1u4Ztm:u52Hl-N2JBVgh5q0IoP5zVDhpONvbHON5mAPztDhtfA"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDcyNiwidXNlciI6OTc4NH0:1u4Ztm:vR-J_Sa0L3jzycYdQWIFhAGZpwCPmVekwf3Drqg7cn8"
        try:
            tests = [('''max_dolzina(['sonce', [['dez', 'veter'], 'sneg'], [[[['mavrica']]]]])''', 7),
                     ('''max_dolzina(['a', ['ab', 'defg'], 'abc'])''', 4),
                     ('''max_dolzina([['a'], ['ab', 'defg'], ['abc']])''', 4),
                     ('''max_dolzina([['a12345678', ['qwrqw', 'qwr', '1']], ['ab', 'defg'], ['abc']])''', 9),
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDcyNywidXNlciI6OTc4NH0:1u4Ztm:cGTjeKU0kno6atIZx_J31cHjzDXlO9FAoTjMncLZC1s"
        try:
            tests = [('''max_dolzina_zadnji(['sonce', [['dez', 'veter'], 'sneg'], [[[['oblak']]]]])''', 'oblak'),
                    ...
                    ('''max_dolzina_zadnji(['sonce', [['dez', 'veter'], 'sneg'], ['oblak']])''', 'oblak'),
                     ...
                     ('''max_dolzina_zadnji(['abcd', 'ab', 'abc'])''', 'abcd'),
                     ('''max_dolzina_zadnji(['abcd', 'ab', 'xyzt'])''', 'xyzt'),
                     ('''max_dolzina_zadnji(['abcd', 'ab', [['xyzt']]])''', 'xyzt'),
                     ('''max_dolzina_zadnji([[[[[[['       ']]]]]]])''', '       '),
                     ('''max_dolzina_zadnji(['', [[['']]]])''', '')]
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDcyOCwidXNlciI6OTc4NH0:1u4Ztm:tI4tv4-k1-2gFxnab7quikjY0-Q_uFvIUAnlzXrQkRo"
        try:
            tests = [('''max_dolzina_vsi(['sonce', [['dez', 'veter'], 'sneg'], [[[['oblak']]]]])''', {'sonce', 'oblak', 'veter'}),
                     ('''max_dolzina_vsi(['sonce', [['dez', 'veter'], 'sneg'], [[[['mavrica']]]]])''', {'mavrica'}),
                     ('''max_dolzina_vsi(['1solnce', [['dez', 'veter_J'], 'sneg'], [[[['mavrica']]]]])''', {'veter_J','1solnce','mavrica'}),
