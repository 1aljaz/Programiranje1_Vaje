...
# =====================================================================@040657=
# 1. podnaloga
# Sestavite funkcijo `podobna(beseda, tab_besed)`,  ki  kot  argument  sprejme
# besedo  in  tabelo  besed  in  kot  rezultat  vrne 
# tisto besedo  iz  `tab_besed`,  v  kateri  se  pojavi  največ  takih  črk,
# ki  se  pojavijo tudi  v  dani besedi. Teh črk je seveda lahko tudi 0.
...
# Če je takih besed več, naj funkcija vrne tisto, ki je prej na vrsti v seznamu.
# Če take sploh besede ni, naj vrne `None`. Lahko predpostavite, da so
# v besedah samo črke.
# 
...
#       'berta'
# 
# `Ana` in `krava` se ujemata v eni črki, namreč črki `a` in ne v dveh, pa čeprav imata
# po dva `a`-ja (tudi če bi napisali `ana`).
# Opomba: vsaka podobnost med `Berto` in `merjascem` ali `kravo`
# ter `Emo` in `zmajem` je zgolj naključna.
# 
...
# =====================================================================@040658=
# 2. podnaloga
# Dana je tabela besed. Napišite funkcijo `razlicni_znaki(tab_besed)`, ki vrne urejeno
# (kot ureja `sorted`)
# tabelo tistih besed, ki imajo vse znake različne. Male in velike črke razlikujemo.
# Vsako besedo upoštevaj le enkrat.
# 
...
def _validate_current_file():
    ...
    if Check.part():
        ...
        try:
            ...
            elif not Check.equal('razlicni_znaki(["abc", "abc"])', ['abc']):
                Check.error("Vsako besedo upoštevamo le enkrat!")
