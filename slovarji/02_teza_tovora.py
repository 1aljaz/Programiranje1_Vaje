...
def teza_tovora(tovor):
    """Vrne tezo tovora. (Vsoto vrednosti v slovarju)"""
    return sum(tovor.values())
# =====================================================================@040666=
# 2. podnaloga
...
def lahko_pelje(tovor, nosilnost):
    """Pove ali tovornjak lahko pelje tovor."""
...
def enaka_tovora(tovor1, tovor2):
    """Vrne True ce sta tovora enaka, drugace False."""
    if len(tovor1) != len(tovor2):
        return False
    
    for kljuc, vrednost in tovor1.items():
        if kljuc not in tovor2:
            return False
        if vrednost != tovor2[kljuc]:
            return False
    return True
# =====================================================================@040668=
# 4. podnaloga
...
# katerih teža je med `min_teza` in `max_teza` (meji vključeni). Zgled:
# 
...
def ustrezen_tovor(tovor, min_teza, max_teza):
    """Vrne slovar vsega tovorja, ki ga 
    tovornjak lahko pelje."""
    sl = {}
    for kljuc, vrednost in tovor.items():
        if vrednost >= min_teza and vrednost <= max_teza:
            sl[kljuc] = vrednost
    return sl
# =====================================================================@040669=
# 5. podnaloga
...
def najkrajsi_opis(tovor):
    """Vrne tabelo najkrajsih kljucev v slovarju."""
    if tovor:
        m = min(len(kljuc) for kljuc, vrednost in tovor.items())
        return sorted([kljuc for kljuc, _ in tovor.items() if len(kljuc) == m])
    return []
# =====================================================================@040670=
# 6. podnaloga
