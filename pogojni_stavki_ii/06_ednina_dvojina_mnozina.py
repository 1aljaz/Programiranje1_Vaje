...
def fin_stanje(stanje:int):
    """V pravilni slovnici napise kolicnio evrov."""
    if stanje < -300:
        return "Ti si navadna zguba!"
    elif stanje >= 1000000:
        return "Tajkun!"

    abs_stanje = abs(stanje)
    zadnja = abs_stanje % 10
    predzadnji = abs_stanje % 100

    if predzadnji in [11, 12, 13, 14]:
        konec = "evrov"
    elif zadnja == 1:
        konec = "evro"
    elif zadnja == 2 and len(str(stanje)) == 1:
        konec = "evra"
    elif zadnja in [3, 4]:
        konec = "evri"
    else:
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDQ2NSwidXNlciI6OTc4NH0:1t9TCH:Wlnp7KN523NNr1AwLKZklcjZ7b9Ln0qW53k3WvKX9dk"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDQ2NiwidXNlciI6OTc4NH0:1t9TCH:31VCYVTU7FBGCNnXYDOpTTY5OP9-jOUf5QhZ6GEUAIk"
        try:
            s = Check.current_part['solution']
            sw = re.sub('[ \n]', '', s)
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDQ2NywidXNlciI6OTc4NH0:1t9TCH:h1UqW7w_2-z9mOeiPZytIhQOuWe1ltlK2rWqNOzNOYY"
        try:
            test_data = [
                ('1', 'o', ''),
