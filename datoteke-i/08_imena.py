...
# take datoteke:
# 
#     Jaka,Peter,Miha,Peter,Anja,Franci
...
def koliko(niz, datoteka):
    sl = {}
    arr = niz.split(',')
    with open(datoteka, 'w', encoding='utf-8') as w:
        for s in arr:
            if s in sl:
                sl[s] += 1
            else:
...
def koliko_iz_datoteke(vhodna, izhodna):
    sl = {}
    with open(vhodna, 'r', encoding='utf-8') as r:
        with open(izhodna, 'w', encoding='utf-8') as w:
            for l in r:
                l = l.strip()
                arr = l.split(',')
                for a in arr:
            ...
            for l in r:
                ...
                for a in arr:
                    if a in sl:
                        ...
                        sl[a] = 1
            sl = sorted(sl.items(), key=lambda t: (-t[1], t[0]))
            for s, k in sl:
                w.write(f"{s} {k}\n")
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDgyMCwidXNlciI6OTc4NH0:1uCBf0:cr67EW_w9EjXGveb_DwH3eFNlvf-9nd4JAKcrvv_5HQ"
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDgyMSwidXNlciI6OTc4NH0:1uCBf0:jp8nfbtAXVlU36T6WkcZoZNMGHG1X_ve7NEcSmWyUik"
        try:
            test_cases = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDgyMiwidXNlciI6OTc4NH0:1uCBf0:3E24bka82rA4Ys-qefrCF_SpVOKmWfu5333h-NvZkL0"
        try:
            test_cases = [
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MDgyMywidXNlciI6OTc4NH0:1uCBf0:vDyFxYF0ZMOKEgC2iqaAIqiSP1QdMIJkV3znB_NigBg"
        try:
            test_cases = [
