...
#
# Pri varčevanju na obroke na začetku vplačamo nek znesek, nato pa vsak
...
# =====================================================================@042459=
# 1. podnaloga
# V bančništvu pogosto navajamo letno obrestno mero, obrestujemo pa mesečno.
# Zato je pred računanjem potrebno obrestno mero primerno spremeniti.
# 
# Napišite funkcijo `mesecna_obrestna_mera(letna_obrestna_mera)`, ki kot
...
# =============================================================================
def mesecna_obrestna_mera(letna_obrestna_mera: float):
    """Vrne letno obrestno mero"""
    return (letna_obrestna_mera / 100) / 12
# =====================================================================@042460=
# 2. podnaloga
# Pri enostavnem obrestovanju s fiksno letno obrestno mero $r$ se
# vedno obrestuje samo glavnica, ne pa tudi obresti. Tako je vrednost
# na računu pri začetnem pologu $A_0$ po $n$ mesecih podana s formulo
# $$A = A_0 * (1 + m * n),$$
...
def enostavno_obrestovanje(polog, letna_obrestna_mera, st_mesecev):
    """Vrne kolicino denarja na racuno po st_mesecev"""
    return polog * (1+mesecna_obrestna_mera(letna_obrestna_mera)*st_mesecev)
# =====================================================================@042461=
# 3. podnaloga
# Pri obrestno obrestnem računu s fiksno letno obrestno mero $r$ se zraven
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MjQ1OSwidXNlciI6OTc4NH0:1t5gA3:pMxJRKOT6ND_ehkyc8dyiiB2XtNwEgEOdnvAFlG1FWk"
        try:
            Check.equal('mesecna_obrestna_mera(12)', 0.01)
            Check.equal('mesecna_obrestna_mera(0)', 0)
            Check.equal('mesecna_obrestna_mera(10)', 0.008333)
            
            _rng = [1287631]
            for i in range(100):
                _rng.append((_rng[-1] * 1103515245 + 12345) % 2 ** 32)
            _m = max(_rng) / 20
            _rng = [x / _m for x in _rng]
            
            for x in _rng:
                Check.secret(mesecna_obrestna_mera(x), x)
        except TimeoutError:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MjQ2MCwidXNlciI6OTc4NH0:1t5gA3:wpXbLC_tWDYeJW7E-loZwinLp3OYNipdXRTXvLRENkI"
        try:
            Check.equal('enostavno_obrestovanje(100, 10, 0)', 100)
            Check.equal('enostavno_obrestovanje(100, 12, 1)', 101)
            Check.equal('enostavno_obrestovanje(100, 10, 12)', 110)
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo0MjQ2MSwidXNlciI6OTc4NH0:1t5gA3:WtfQonEORKF_MQFgzzLZdCaiHCX64fYW8ocPORrMPZc"
        try:
            Check.equal("obrestno_obrestovanje(100, 10, 0)", 100)
            Check.equal("obrestno_obrestovanje(100, 10, 1)", 100.8333333)
            Check.equal("obrestno_obrestovanje(100, 12, 12)", 112.682503)
